...
export type ContractOptionType = 'phase' | 'status';

export type ContractOptionRecord = {
  id: string;
...
export type AccountStatusOptionRecord = {
  id: string;
  value: string;
  label: string;
  sortOrder: number;
...
const generateId = (prefix: string): string => `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
...
const normalizePaletteId = (value: unknown): string => {
...
const normalizeChatLoginId = (value: unknown): string | null => {
  const normalized = asString(value).trim().toUpperCase();
  return normalized ? normalized : null;
};

const normalizePasswordInput = (value: unknown): string => {
...
const generateNextPaletteId = async (): Promise<string> => {
  ...
  const used = new Set(
    result.rows
      .map((row) => asString((row as Row).palette_id).toUpperCase())
...
const normalizeContractOption = (row: Row): ContractOptionRecord => ({
  id: asString(row.id),
  optionType: asString(row.option_type ?? row.type) as ContractOptionType,
...
const normalizeServiceSubscription = (row: Row): ServiceSubscriptionRecord => ({
  id: asString(row.id),
  accountId: asString(row.account_id),
...
export const listAccounts = async (): Promise<AccountRecord[]> => {
  ...
  `;
  return result.rows.map((row: unknown) => normalizeAccount(row as Row));
};

export const upsertAccount = async (input: Partial<AccountRecord> & { chatPassword?: string | null }): Promise<AccountRecord> => {
  await ensureTables();
  ...
  if (matchedId) id = asString(matchedId);
  ...
  const existingAccountRes = await sql`
    ...
    LIMIT 1
  `;
  const existingAccount = (existingAccountRes.rows?.[0] || {}) as Row;
  ...
  const chatLoginId = hasChatLoginId
    ? normalizeChatLoginId((input as { chatLoginId?: string | null }).chatLoginId)
    : asNullableString(existingAccount.chat_login_id);

  const nextPasswordInput = hasChatPassword ? normalizePasswordInput((input as { chatPassword?: string | null }).chatPassword) : '';
...
export const listPlans = async (includeInactive = false): Promise<ServicePlanRecord[]> => {
  await ensureTables();
  ...
  return result.rows.map((row: unknown) => normalizePlan(row as Row));
};

export const upsertPlan = async (input: Partial<ServicePlanRecord>): Promise<ServicePlanRecord> => {
  await ensureTables();
  ...
  return normalizePlan(result.rows[0] as Row);
};

export const listContracts = async (options?: { accountId?: string; paletteId?: string; activeOn?: string }): Promise<ContractRecord[]> => {
  await ensureTables();
  ...
  if (accountId) {
    const result = activeOn
      ...
      : await sql`
          ...
          ORDER BY start_date DESC, updated_at DESC
        `;
    return result.rows.map((row: unknown) => normalizeContract(row as Row));
  }
...
export const upsertContract = async (input: Partial<ContractRecord>): Promise<ContractRecord> => {
  ...
  return normalizeContract(result.rows[0] as Row);
};

export const listServiceSubscriptions = async (options?: { accountId?: string; paletteId?: string; activeOn?: string }) => {
  await ensureTables();
  ...
  if (accountId) {
    const result = activeOn
      ...
      : await sql`
          ...
          ORDER BY start_date DESC, updated_at DESC
        `;
    return result.rows.map((row: unknown) => normalizeServiceSubscription(row as Row));
  }
...
export const upsertServiceSubscription = async (input: Partial<ServiceSubscriptionRecord>) => {
  await ensureTables();
...
export const getPaletteSummary = async (paletteId: string, activeOn?: string) => {
  ...
  const account = normalizeAccount(accountRes.rows[0] as Row);
...
export const getPaletteServices = async (paletteId: string, activeOn?: string) => {
  const summary = await getPaletteSummary(paletteId, activeOn);
  if (!summary) return null;

  const serviceKeys = Array.from(new Set(summary.services.map((item: ServiceSubscriptionRecord) => item.serviceKey)));
...
export const deleteServiceSubscription = async (id: string) => {
  await ensureTables();
  await sql`DELETE FROM service_subscriptions WHERE id = ${id}`;
};

export const listContractOptions = async (optionType: ContractOptionType, includeInactive = false): Promise<ContractOptionRecord[]> => {
  await ensureTables();
  const result = includeInactive
    ? await sql`
        SELECT
    ...
    : await sql`
        ...
        ORDER BY sort_order ASC, updated_at DESC
      `;
  return result.rows.map((row: unknown) => normalizeContractOption(row as Row));
};

export const upsertContractOption = async (input: Partial<ContractOptionRecord>): Promise<ContractOptionRecord> => {
  await ensureTables();
  const optionType = asString(input.optionType) as ContractOptionType;
  ...
  return normalizeContractOption(result.rows[0] as Row);
};

export const deleteContractOption = async (id: string) => {
  await ensureTables();
  await sql`DELETE FROM contract_options WHERE id = ${id}`;
};

export const listAccountStatusOptions = async (includeInactive = false): Promise<AccountStatusOptionRecord[]> => {
  await ensureTables();
  const result = includeInactive
    ? await sql`
    ...
    : await sql`
        ...
        ORDER BY sort_order ASC, updated_at DESC
      `;
  return result.rows.map((row: unknown) => normalizeAccountStatusOption(row as Row));
};

export const upsertAccountStatusOption = async (input: Partial<AccountStatusOptionRecord>): Promise<AccountStatusOptionRecord> => {
  await ensureTables();
  ...
  return normalizeAccountStatusOption(result.rows[0] as Row);
};

export const deleteAccountStatusOption = async (id: string) => {
  await ensureTables();
  await sql`DELETE FROM account_status_options WHERE id = ${id}`;
};

export const hasChatLoginId = async (loginIdInput: string): Promise<boolean> => {
  await ensureTables();
...
export const verifyChatLogin = async (loginIdInput: string, passwordInput: string) => {
  await ensureTables();
  const loginId = normalizeChatLoginId(loginIdInput);
